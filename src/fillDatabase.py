...
def createStockHistoricalDataTable():
    ...
    InteractionDB("src/database.db").insertInDB(
        dataframeTrasposed, "stockHistoricalData"
    )
...
def createExchangesRateAnagraphicDataTable():
    ...
    dataframe = pd.DataFrame(
        data=exchangesRateAnagraphicDataList,
        columns=["agency", "description", "displaySymbol", "symbol"],
    )
    InteractionDB("src/database.db").insertInDB(
        dataframe, "exchangesRateAnagraphicData"
    )
...
def createExchangeRatesHistoricalDataTable():
    ...
    for symbol in c.exchangeSymbols:
        url = (
            "https://finnhub.io/api/v1/forex/candle?symbol="
            + symbol
            + "&resolution=W&from="
            + FROM
            + "&to="
    ...
    dataframe = pd.DataFrame(
        data=exchangeRatesHistoricalDataList, index=c.exchangesDisplaySymbols
    )
    dataframeTrasposed = dataframe.transpose()
    InteractionDB("src/database.db").insertInDB(
        dataframeTrasposed, "exchangeRatesHistoricalData"
    )
